...
int checkValidity(int number, int puzzle[][9], int row, int column) {
    ...
    if(puzzle[row1+tempRow][col1+tempCol] == number || 
    puzzle[row2+tempRow][col1+tempCol] == number || 
    puzzle[row1+tempRow][col2+tempCol] == number || 
    puzzle[row2+tempRow][col2+tempCol] == number) // Check remaining cells in the square for potential value.
    {
        return 0;
    }
    return 1;
